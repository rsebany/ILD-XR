...
export function View2DPanelCenterColumn({
  ...
  const pngSliceUrl = useMemo(() => {
    ...
    const params = new URLSearchParams({
      ...
      include_overlay: showOverlay ? "true" : "false",
      denoise: "false",
    });
    if (showOverlay) {
      params.set("overlay_opacity", clampedOverlayAlpha.toFixed(2));
    }
    return `${getApiBaseUrl()}/studies/${studyId}/slices/${safeSliceIndex}?${params}`;
