...
export function preloadDicomSliceTexture(studyId: string, slice: number): Promise<void> {
  ...
  const url = `${getApiBaseUrl()}/studies/${encodeURIComponent(studyId)}/slices/${slice}?${params}`;
  return new Promise((resolve, reject) => {
